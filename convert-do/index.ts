...
Steps 0-7 stream in-memory; steps 8-10 are directory-level batch passes that
read the materialized step{N-1} folder.

Usage: tsx convert-do/index.ts [options]

Options:
  --from <step>     Start from step N (0-10, default: 0)
  --to <step>       Process up to step N (0-10, default: 10)
...
Examples:
  tsx convert-do/index.ts                  # Full pipeline, output in step10/
  tsx convert-do/index.ts --to 5           # Steps 0-5, output in step5/
  tsx convert-do/index.ts --from 8 --to 10 # Batch steps 8-10 (reads step7/)
...
`);
  ...
  if (values.step) {
    ...
    if (isNaN(step) || step < 0 || step > 10) {
      consola.error(`Invalid --step value: ${values.step}. Must be 0-10.`);
      process.exit(1);
    }
    fromStep = step;
    toStep = step;
    ...
    if (isNaN(fromStep) || fromStep < 0 || fromStep > 10) {
      consola.error(`Invalid --from value: ${values.from}. Must be 0-10.`);
      process.exit(1);
    }
    if (isNaN(toStep) || toStep < 0 || toStep > 10) {
      consola.error(`Invalid --to value: ${values.to}. Must be 0-10.`);
      process.exit(1);
    }
    if (fromStep > toStep) {
      consola.error(
