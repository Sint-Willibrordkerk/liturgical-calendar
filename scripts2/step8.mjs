...
const STEP7_INPUT = join(PROJECT_BASE, ".divinum-officium", "step7");
const STEP8_OUTPUT = join(PROJECT_BASE, ".divinum-officium", "step8");
...
async function main() {
  await rm(STEP8_OUTPUT, { recursive: true, force: true });
  await mkdir(STEP8_OUTPUT, { recursive: true });

  const files = await collectYmlFiles(STEP7_INPUT);
  ...
  const { processed, errors } = await runBatched(
    ...
    CONCURRENCY,
    async (rel) => {
      const inputPath = join(STEP7_INPUT, rel);
      const outputPath = join(STEP8_OUTPUT, rel);
      const raw = await readFile(inputPath, "utf-8");
      const obj = parse(raw);
      if (typeof obj !== "object" || obj === null || Array.isArray(obj)) {
        throw new Error("Expected object");
      }
      const out = await transformObject(obj, rel, STEP7_INPUT);
      await ensureDir(outputPath, mkdirCache);
      await writeFile(outputPath, stringify(out), "utf-8");
    }
  );

  console.log(`Step 8 done. ${processed} files in ${STEP8_OUTPUT}, ${errors} errors`);
}
